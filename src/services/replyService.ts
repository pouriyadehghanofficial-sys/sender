...
import { sendBaleMessage, sendBalePhoto } from "../providers/baleBotClient";
...
import { enqueueDebouncedReply } from "../queue/replyQueue";
...
export async function replyToChat(chatId: string): Promise<boolean> {
  try {
    return await runExclusive(`reply:${chatId}`, async () => {
      ...
      if (result.photoUrl) {
        try {
          const photoRes = await sendBalePhoto({ botToken }, result.chatId, result.photoUrl);
          if (photoRes && photoRes.ok === false) throw new Error(JSON.stringify(photoRes));
        } catch (err) {
          // ارسال عکس اختیاری است؛ اگر شکست خورد، پاسخ متنی همچنان باید برسد
          recordError("sendPhoto", err);
          console.warn(`[reply] ارسال عکس محصول به chatId=${result.chatId} ناموفق بود:`, (err as Error)?.message);
        }
      }
...
export function scheduleReply(chatId: string) {
  // اگر Redis تنظیم شده، debounce با صف واقعی انجام می‌شود (روی ری‌استارت سرور هم دوام دارد).
  // نکته: در این حالت برای سادگی فقط DEBOUNCE_MS اعمال می‌شود (سقف MAX_WAIT_MS نادیده گرفته
  // می‌شود)؛ چون هر پیام جدید timer را کامل از نو تنظیم می‌کند، مثل قبل عملاً به‌ندرت به آن سقف می‌رسید.
  if (isQueueEnabled) {
    void enqueueDebouncedReply(chatId, DEBOUNCE_MS).catch((err) =>
      console.error(`[reply] خطا در صف‌بندی Redis برای chatId=${chatId}:`, err)
    );
    return;
  }
